...
// addRacingStripe - Add the stripe or Matte finish to vehicle
function addRacingStripe(stripe) {
  let imgURL = getImageID();
  let arrFile = makeArrayFromFile(getImageURL());
  let origStripe = arrFile[4];
  arrFile[4] = stripe;
  let url = concatArray(arrFile);
  imgSwitch(url);
}
